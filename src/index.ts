 ...
 */
export class LazyRefMarker<T = any> {
 ...
 * Usage: @Lazy(ServiceB) private serviceB: LazyRef<ServiceB>
 */
export function Lazy<T>(token: Constructor<T> | InjectionToken<T>): ParameterDecorator {
  return (
    target: Object,
...
export type ForwardRef<T = any> = LazyRefMarker<T>;
...
export function Inject(token: InjectionToken | LazyRefMarker): ParameterDecorator {
  return (
    target: Object,
    _propertyKey: string | symbol | undefined,
    parameterIndex: number
...
export class Container {
  ...
  private readonly providers = new Map<InjectionToken | Constructor<any>, Provider>();

  // Cache instantiated services
  private readonly instances = new Map<InjectionToken | Constructor<any>, any>();

  // Track resolution to detect circular dependencies
  private readonly resolutionStack = new Set<InjectionToken | Constructor<any>>();

  // Cache for dependency weights
  private readonly weightCache = new Map<InjectionToken | Constructor<any>, number>();
  ...
  public register<T>(provider: Provider<T>): void {
    if (this.isClassProvider(provider)) {
      console.log(`Registering class provider: ${String(provider.provide)}`);
      this.providers.set(provider.provide, provider);
    } else if (this.isValueProvider(provider)) {
      console.log(`Registering value provider: ${String(provider.provide)}`);
      this.providers.set(provider.provide, provider);
      this.instances.set(provider.provide, provider.useValue);
    } else if (this.isFactoryProvider(provider)) {
      console.log(`Registering factory provider: ${String(provider.provide)}`);
      this.providers.set(provider.provide, provider);
    } else {
      // Plain class constructor
      console.log(`Registering class: ${(provider as Constructor).name}`);
      this.providers.set(provider as Constructor, provider);
    }
  }
   ...
   * Returns undefined if not yet resolved
   */
  public getInstance<T>(token: InjectionToken<T> | Constructor<T>): T | undefined {
   ...
   * Throws if not yet resolved
   */
  public getInstanceOrThrow<T>(token: InjectionToken<T> | Constructor<T>): T {
   ...
   */
  public getInstancesMap(): ReadonlyMap<InjectionToken | Constructor<any>, any> {
   ...
   */
  public getProvidersMap(): ReadonlyMap<InjectionToken | Constructor<any>, Provider> {
   ...
   */
  public resolve<T>(constructor: Constructor<T>, skipCircularCheck?: boolean): Promise<T>;
  public resolve<T>(token: InjectionToken<T>, skipCircularCheck?: boolean): Promise<T>;
  public async resolve<T>(token: InjectionToken<T> | Constructor<T>, skipCircularCheck = false): Promise<T> {
   ...
   */
  private async instantiateFactory<T>(provider: FactoryProvider<T>): Promise<T> {
   ...
   */
  private getClassDependencies(target: Constructor<any>): (InjectionToken | Constructor<any>)[] {
    const injectionTokens: any[] =
      Reflect.getMetadata("inject:tokens", target) || [];
   ...
   */
  public async resolveAll(): Promise<Map<InjectionToken | Constructor<any>, any>> {
    ...
    for (const token of this.providers.keys()) {
      const provider = this.providers.get(token);
      if (provider && !this.isValueProvider(provider)) {
        const target = this.isClassProvider(provider) ? provider.useClass : provider as Constructor<any>;
        const injectionTokens: any[] = Reflect.getMetadata("inject:tokens", target) || [];

        injectionTokens.forEach((injectToken) => {
          // Check for new @Lazy pattern
          if (injectToken && typeof injectToken === 'object' && '__lazyToken' in injectToken) {
    ...
    for (const { token, weight } of sorted) {
      if (!this.instances.has(token) && lazyTargets.has(token)) {
        console.log(`[Lazy, Weight ${weight}] Resolving: ${this.getTokenName(token)}`);
        try {
          await this.resolve(token as any);
        } catch (error: any) {
          console.log(`  ✗ Failed: ${error.message}`);
   ...
   */
  public getDependencyGraph(): Map<string, { weight: number; dependencies: string[] }> {
